...
class ContactoController {
  async create_contacto(req, res) {
    try {
      ...
      const existe = await _models.default.contacto.findOne({
        where: {
          numero_contacto: req.body.numero_contacto
        }
      });
      if (!existe) {
        const Contacto = await _models.default.contacto.create({
        ...
        let nuevoContacto = await _models.default.contacto.findOne({
        ...
        res.status(422).json({
          status: 422,
          mensaje: "El sistema no puede completar el registro: el numero de contacto ya existe",
          error: true
        });
      }
    } catch (error) {
  ...
  async read_contacto(req, res) {
    try {
      ...
      let Contacto = await _models.default.contacto.findOne({
        attributes: ["id_contacto", "nombre_apellidos", "numero_contacto", "correo_contacto", "nombre_empresa", "profesion", "intereses", "observaciones"],
        where: {
  ...
  async readAll_contacto(req, res) {
    try {
      ...
      let Contacto = await _models.default.contacto.findAll({
        attributes: ["id_contacto", "nombre_apellidos", "numero_contacto", "correo_contacto", "nombre_empresa", "profesion", "intereses", "observaciones"],
        where: {
          estado: true
        },
  ...
  async update_contacto(req, res) {
    try {
      ...
      if (existe) {
        ...
        let contactoActualizado = await _models.default.contacto.findOne({
          attributes: ["id_contacto", "nombre_apellidos", "numero_contacto", "correo_contacto", "nombre_empresa", "profesion", "intereses", "observaciones"],
          where: {
            id_contacto: req.body.id_contacto,
            estado: true
  ...
  async readAll_tipoSeguimiento(req, res) {
    try {
      ...
      let Tipo_seguimiento = await _models.default.tipo_seguimiento.findAll({
  ...
  async readAll_estadoContacto(req, res) {
    try {
      ...
      let Estado = await _models.default.estado_contacto.findAll({
        attributes: ["id_estado_contacto", "nombre_estado"]
      });
      if (Estado) {
        res.status(200).json({
    ...
    try {
      ...
      for (const contacto of listaContactos) {
        ...
        if (existeContacto) {
          ...
          const historico = await _models.default.contacto.create({
...
const contactoController = exports.contactoController = new ContactoController();