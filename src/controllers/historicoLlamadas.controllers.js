...
class HistoricoController {
  async registrarCambiosLlamada(req, res) {
    try {
      ...
      const Cambios = await models.historial_seguimiento.create({
  ...
  async verCambiosLlamada(req, res) {
    try {
      ...
      let CambiosLLamada = await models.historial_seguimiento.findAll({
        attributes: [
          "id_historico",
          "observacion_llamada",
          "fecha_seguimiento",
      ...
      for (const contacto of listaContactos) {
        ...
        if (existeContacto) {
          ...
          if (existeHistorico) {
            ...
            const historico = await models.historico_llamadas.create({
              id_grupo_seguimiento: id_grupo_seguimiento,
              id_contacto: existeContacto.id_contacto,
            });
          }
  ...
  async actualizarSeguimientoContacto(req, res) {
    ...
    try {
      const existeContacto = await models.contacto.findOne({
        where: { numero_contacto: dataToUpdate.Contactos.numero_contacto },
      });
      const existeHistorico = await models.historico_llamadas.findOne({
        where: { id_historico: dataToUpdate.id_historico },
      });
      ...
      if (existeContacto && existeHistorico) {
        const contacto = await models.contacto.update(
          {
            ...
            id_estado_contacto:
              dataToUpdate.Contactos.Estado.id_estado_contacto,
          },
          {
            where: {
        ...
        const historico = await models.historico_llamadas.update(
          {
            ...
            id_tipo_seguimiento:
              dataToUpdate.TipoSeguimiento.id_tipo_seguimiento,
        ...
        if(existeContacto.observacion_llamada != dataToUpdate.observacion_llamada){
          ...
          const cambios = await models.historial_seguimiento.create({
            id_historico: dataToUpdate.id_historico,
            observacion_llamada: dataToUpdate.observacion_llamada,
            fecha_seguimiento: models.sequelize.literal('CURRENT_TIMESTAMP'),
          })
        }

        let unContactoSeguimiento = await models.historico_llamadas.findOne({
          attributes: [
            "id_historico",
            "id_grupo_seguimiento",
            "fecha_actualizacion",
          ...
          include: [
            {
              ...
              where: { estado: true, id_contacto: dataToUpdate.Contactos.id_contacto },
              include: [
                {
                  model: models.sexo,
                  as: "Sexo_contacto",
  ...
  async asignarContactoEnSeguimiento(req, res) {
    try {
      ...
      const { id_grupo_seguimiento, id_capacitacion, listaContactos } = req.body;
      for (const contacto of listaContactos) {
        const existeContacto = await models.contacto.findOne({
        ...
        if (existeContacto) {
          ...
          const existeHistorico = await models.historico_llamadas.findOne({
            include: [
              {
                model: models.grupo_seguimiento,
                as: "Seguimiento",
                where: {
                  // id_grupo_seguimiento: id_grupo_seguimiento,
  ...
  async eliminarContactoEnSeguimiento(req, res) {
    try {
      //console.log(req.body)
      const existe = await models.historico_llamadas.findOne({
        where: { id_historico: req.params.id_historico },
      });
      if (existe) {
        const Contacto = await models.historico_llamadas.destroy(
          {
            where: { id_historico: req.params.id_historico },
          }
        );
