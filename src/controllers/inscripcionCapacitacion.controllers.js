...
class InscripcionCapacitacionController {
  async inscribir_capacitacion(req, res) {
    try {
      ...
      let inscripcion = await models.informe_inscripciones.findOne({
        where: {
      ...
      if (!inscripcion) {
        ...
        const Inscripcion = await models.informe_inscripciones.create({
          id_participante: req.body.id_participante,
  ...
  async inscribirListaParticipantes(req, res) {
    try {
      ...
      for (const inscripcion of inscripciones) {
        const existente = await models.informe_inscripciones.findOne({
          where: {
      ...
      if (inscripcionesDuplicadas.length > 0) {
        return res.status(422).json({
          status: 422,
          mensaje:
            "Algunos participantes ya están inscritos en esta capacitación y se ha cancelado la inscripción de los demás",
      ...
      for (const inscripcion of inscripciones) {
        await models.informe_inscripciones.create({
          id_participante: inscripcion.id_participante,
          id_capacitacion: inscripcion.id_capacitacion,
        });
      }
  ...
  async getParticipantesNoInscritos(req, res) {
    try {
      //console.log(req.body)
      const participantesInscritos = await models.informe_inscripciones.findAll(
        {
          attributes: ["id_participante"],
          where: { id_capacitacion: req.params.id_capacitacion },
        }
      );
      let participantesNoInscritos = await models.participante.findAll({
        attributes: ["id_participante"],
        where: {
          estado: true,
          id_participante: {
            [models.Sequelize.Op.notIn]: participantesInscritos.map(
  ...
  async eliminarUnParticipanteInscrito(req, res) {
    try {
      //console.log(req.body)
      const existe = await models.informe_inscripciones.findOne({
        where: {
      ...
      if (existe) {
        const eliminar = await models.informe_inscripciones.destroy({
          where: {
            id_inscripcion: req.params.id_inscripcion,
          },
        });
...
export const inscripcionCapacitacionController =
  new InscripcionCapacitacionController();
